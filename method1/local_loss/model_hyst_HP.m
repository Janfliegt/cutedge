...
%             number_of_measurement = length(eval(struct_name).(width_names{i}).(frequency_name).Jmax_T ); 
...
%                 J_tmp = eval(struct_name).(width_names{i}).(frequency_name).Jmax_T(j);
                if (J_tmp < J_min_a1) || (J_tmp > J_max_a1)
                    continue;
                end     
                
                P_hyst_measurement = HystFunktion(J_tmp); 
%                 H_tmp = eval(struct_name).(width_names{i}).(frequency_name).Hmax_A_m(j);
