function [] = data_import_simplification_plot_check(original_data_dir, number_bad_lines, cur_dir, processed_data_dir)
...
eval(['cd ' path_actual])
save(fullfile(processed_data_dir, 'raw_data.mat'), strcat(filename_actual, '_messung_raw'))
...
for i = 1 : number_of_width
    ...
    for j = 1 : number_of_frequency
        ...
        try
            table_simplified = table_tmp(:,{'Sample', 'Jmax_T_', 'Hmax_A_m_', 'Bmax_T_', 'x_r', 'Frequency_Hz_', 'Ps_W_kg_'}); % todo sample oder probe
        catch
            table_simplified = table_tmp(:,{'Probe', 'Jmax_T_', 'Hmax_A_m_', 'Bmax_T_', 'x_r', 'Hmax_A_m_', 'Frequenz_Hz_', 'Ps_W_kg_'}); % todo sample oder probe    
        end
        table_simplified.Properties.VariableNames = {'Sample', 'Jmax_T', 'Hmax_A_m', 'Bmax_T', 'mu_relative', 'Frequenz_Hz', 'Ps_W_kg'}; % to do sample oder probe
        eval(strcat(filename_actual,'_messung_simplified','.', width_name_text, '.', 'f', frequency_name, 'Hz',  '=', 'table_simplified'  ) )
    end

end

save(fullfile(processed_data_dir, 'simplified_data.mat'), strcat(filename_actual,'_messung_simplified')) % sorted simplified data save
...
save_dir =fullfile(processed_data_dir,  'plot_original_data');
mkdir(save_dir)
...
%     end
%     curve_reasonable=1;
    save_path = fullfile(save_dir,convertStringsToChars(filename));
...
close all
%% check matrix 
save_checkmatrix_dir = fullfile(processed_data_dir, 'check_measure_data');
if ~exist(save_checkmatrix_dir, 'dir')
...
for i = 1 : number_of_frequency
    ...
    %
    save_check_path = fullfile(save_checkmatrix_dir,convertStringsToChars(frequency_name));
